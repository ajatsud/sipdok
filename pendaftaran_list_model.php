...
class pendaftaran_list_model
{
	...
	{    
		...
		$where .= ' and ( select b.jenkel 
		                    from pasien b 
		                   where b.id = a.id_pasien ) like :jenkel';
		...
		if($this->keyword){
			...
			$where .= ' and (
				(( select b.nama from pasien b where b.id = a.id_pasien ) like :keyword ) or
				(( select b.alamat from pasien b where b.id = a.id_pasien ) like :keyword )
			)';
	...
	function delete($id)
	{
		$st = $this->pdo->prepare('select count(*) as total from pendaftaran where id = :id');
		$st->execute(['id' => $id]);
		
		$rs = $st->fetch(PDO::FETCH_ASSOC);
		
		if($rs['total'] == 0)
            return new self($this->pdo, $this->keyword, $this->jenkel, $this->period, ['Data pendaftaran dengan ID ' . $id . ' tidak ditemukan'], $this->messages);
		
        try{
            
            $st = $this->pdo->prepare('delete from pendaftaran where id = :id');
            $st->execute(['id' => $id]);
            
            return new self($this->pdo, $this->keyword, $this->jenkel, $this->period, $this->errors, ['Data pendaftaran dengan ID ' . $id . ' berhasil dihapus']);
        }catch (PDOException $e){
