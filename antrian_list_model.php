	...
	{    
		...
		if(strlen($this->period) == 2){ // dd hari ini
			...
			try{
				...
				$st = $this->pdo->prepare('select min(insert_dt) as min_dt,
		...
		if($this->keyword){
			...
			$where .= ' and (   (( select b.nama from pasien b where b.id = a.id_pasien ) like :keyword ) or
                				(( select b.alamat from pasien b where b.id = a.id_pasien ) like :keyword )    )';
            
			$param['keyword'] = '%' . $this->keyword . '%';
		}
		
		// yang sudah diperiksa dokter, jangan tampilkan
		$where .= ' and a.id not in (select b.id_pendaftaran
		                               from periksa b
		                              where b.insert_dt between :from_dt and :to_dt)';
